...
public class UnityServerSessionService : IUnityServerSessionService
{
    ...
    public async Task<DsInfo> AssignSessionAsync(string sessionName, int stageId, int expectedPlayers)
    {
        ...
        var requestBody = $"{{\"sessionName\":\"{sessionName}\",\"stageId\":{stageId},\"expectedPlayers\":{expectedPlayers}}}";
        ...
        _logger.LogDebug(
            "DS へセッション開始リクエスト送信: url={Url}, sessionName={SessionName}, stageId={StageId}, expectedPlayers={ExpectedPlayers}",
            url, sessionName, stageId, expectedPlayers);
