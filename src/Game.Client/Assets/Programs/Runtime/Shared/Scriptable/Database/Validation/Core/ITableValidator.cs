...
namespace Game.Shared.Scriptable.Database.Validation
{
    ...
    /// （直接実装すると <see cref="IRecordValidator{TRecord}"/> 群の実行契約を満たせないため、
    /// <see cref="ValidationExecutor"/> が構成エラーとして検出する）。
    /// </summary>
    public interface ITableValidator<TRecord> : ITableValidator
    {
        ValidationResult ValidateAll(IReadOnlyList<IRecordValidator<TRecord>> recordValidators, IRecordGetter recordGetter);
    }
}
#endif
