...
{
    [CreateAssetMenu(menuName = "Scriptable Database/Table/HorrorCraftMasterTable")]
    public sealed partial class HorrorCraftMasterTable : ScriptableTable<HorrorCraftMaster>
    {
        // PrimaryKey: Id
        private static readonly System.Func<HorrorCraftMaster, int> _pkSel = r => r.Id;
        private static readonly IComparer<int> _pkCmp = Comparer<int>.Default;

        public HorrorCraftMaster FindById(int key)
        {
            return FindUnique(records, key, _pkSel, _pkCmp);
        }

        public bool TryFindById(int key, out HorrorCraftMaster record)
        {
            return TryFindUnique(records, key, _pkSel, _pkCmp, out record);
        }

        public HorrorCraftMaster FindClosestById(int key, bool lower = true)
        {
            return FindClosest(records, key, _pkSel, _pkCmp, lower);
        }

        public ScriptableTableRecords<HorrorCraftMaster> FindRangeById(int min, int max, bool asc = true)
        {
