...
namespace Game.Shared.Scriptable.Database.Validation
{
    ...
    /// レコード単位の検証だけなら <see cref="IRecordValidator{TRecord}"/> の実装だけで足りる。
    /// </summary>
    public class TableValidator<TRecord> : ITableValidator<TRecord>
    {
        ...
        // 非 virtual。派生が実行順序を差し替えて recordValidators を素通りさせられないようにする。
        public ValidationResult ValidateAll(IReadOnlyList<IRecordValidator<TRecord>> recordValidators, IRecordGetter recordGetter)
        {
